...
The function should return an array of the merged intervals so that
no two intervals overlap and all the intervals collectively cover all the time ranges in the original input.

Input: intervals = [[3,5],[1,4],[7,9],[6,8]]
Output: [[1,5],[6,9]]
Explanation: The intervals [3,5] and [1,4] overlap and are merged into [1,5].
Similarly, [7,9] and [6,8] overlap and are merged into [6,9].
*/

// solution 1)cur 和 last 重叠：更新 last 的结束位置    2)不重叠：把 cur 放进结果
func mergeIntervals(intervals [][]int) [][]int {
	...
	for _, interval := range intervals {
		...
		} else { // overlap, just merged last interval
